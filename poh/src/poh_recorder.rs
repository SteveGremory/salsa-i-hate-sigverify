...
impl PohRecorder {
    ...
    pub(crate) fn record(
        ...
        bank_id: BankId,
        mut mixins: Vec<Hash>,
        mut transaction_batches: Vec<Vec<VersionedTransaction>>,
        ...
        // This ensures all-or-nothing recording for blocks.
        ...
        loop {
            ...
            // Process mixins in batches that fit within the current tick
            ...
            if mixed_in {
                debug_assert_eq!(self.entries.len(), batch_len);
                for (entry, transactions) in self
                    .entries
                    .drain(..)
                    .zip(transaction_batches.drain(..batch_len))
                {
                ...
                // We checked available hashes before the loop, so we should always drain all
                if mixins.is_empty() {
                    let poh_lock = self.poh.lock().unwrap();
