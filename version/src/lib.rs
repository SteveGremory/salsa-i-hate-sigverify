...
impl TryFrom<ClientId> for u16 {
    ...
    fn try_from(client: ClientId) -> Result<Self, Self::Error> {
        match client {
            ...
            ClientId::Unknown(client @ 10u16) => Err(format!("Invalid client: {client}")),
...
mod test {
    ...
    fn test_client_id() {
        ...
        assert_eq!(ClientId::from(10u16), ClientId::Harmonic);
        for client in 4u16..=9u16 {
            assert_eq!(ClientId::from(client), ClientId::Unknown(client));
        }
        for client in 11u16..=u16::MAX {
            assert_eq!(ClientId::from(client), ClientId::Unknown(client));
        }
        ...
        assert_eq!(u16::try_from(ClientId::Harmonic), Ok(10u16));
        ...
        assert_eq!(
            u16::try_from(ClientId::Unknown(10u16)),
            Err(format!("Invalid client: 10"))
        );
        for client in 4u16..=9u16 {
