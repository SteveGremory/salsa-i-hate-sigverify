...
impl Default for DevinScheduler {
    fn default() -> Self {
        Self {
            // TODO: tune initial allocations
            running_locks: AHashMap::with_capacity(10_000),
...
impl std::fmt::Debug for DevinScheduler {
...
impl DevinScheduler {
    ...
    #[inline(always)]
    fn is_blocked(tx: &impl SVMMessage, locks: &AHashMap<[u8; 32], u32>) -> bool {
        for (i, account) in tx.account_keys().iter().enumerate() {
            if let Some(&lock) = locks.get(account.as_array()) {
                ...
                if lock == u32::MAX || (lock != 0 && tx.is_writable(i)) {
                    return true;
                }
            }
        }
    ...
    #[inline(always)]
    fn lock_accounts(tx: &impl SVMMessage, locks: &mut AHashMap<[u8; 32], u32>) {
        for (i, account) in tx.account_keys().iter().enumerate() {
            let write = tx.is_writable(i);
            locks
    ...
    #[inline(always)]
    fn unlock_accounts(tx: &impl SVMMessage, locks: &mut AHashMap<[u8; 32], u32>) {
        for (i, account) in tx.account_keys().iter().enumerate() {
    ...
    pub fn init<T: TransactionWithMeta>(&mut self, transactions: &[T]) {
        // info!("init():\n{self:?}");
        self.indices.clear();
    ...
    /// }
    /// ```
    pub fn pop<T: TransactionWithMeta>(
        &mut self,
        transactions: &[T],
        bank: &Bank,
    ) -> Option<Range<usize>> {
        ...
        for &i in self.indices.iter().skip(self.next).take(Self::MAX_RANGE) {
            ...
            if Self::is_blocked(tx, &self.running_locks)
                || Self::is_blocked(tx, &self.skipped_locks)
            {
            ...
            cost += CostModel::calculate_cost(tx, &bank.feature_set).sum();
    ...
    pub fn finish<T: TransactionWithMeta>(&mut self, range: Range<usize>, transactions: &[T]) {
        // info!("finish():\n{self:?}");
        ...
        for i in range {
            let tx = &transactions[i];
            Self::unlock_accounts(tx, &mut self.running_locks);
        }
